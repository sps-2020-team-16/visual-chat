...
export class LAppModel extends CubismUserModel {
  ...
  private setupModel(setting: ICubismModelSetting): void {
    ...
    const loadCubismMotion = (): void => {
      ...
      // EXTRA CHANGES HERE:
      ////////////////////////////////////////////
      // EXPOSE_EXPRESSION_FUNCS
      ////////////////////////////////////////////
      LAppDefine._global.AVATAR_MANIPULATE_EXPRESSIONS = {};
      for (let i = 0; i < this._expressions.getSize(); i++) {
        const name: string = this._expressions._keyValues[i].first;
        const key: string = `expression:${ this._expressions._keyValues[i].first }`;
        LAppDefine._global.AVATAR_MANIPULATE_EXPRESSIONS[ key ] = () => {
      ...
      // EXPOSE_MOTION_FUNCS
      ////////////////////////////////////////////
      LAppDefine._global.AVATAR_MANIPULATE_MOTIONS = {}
      let exposeMotionGroupFuncs = ( group )=>{
        for (let i = 0; i < this._modelSetting.getMotionCount(group); i++) {
          const no: number = i;
          const key: string = `motion:${ group }:${ no }`;
          LAppDefine._global.AVATAR_MANIPULATE_MOTIONS[ key ] = () => {
            this.startMotion(group, no, LAppDefine.PriorityForce, null);
          }
        }
      }

      exposeMotionGroupFuncs( LAppDefine.MotionGroupIdle );
      exposeMotionGroupFuncs( LAppDefine.MotionGroupTapBody );

      // console.log(LAppDefine.onANewModelLoaded)
      if( LAppDefine.onANewModelLoaded ){
        LAppDefine.onANewModelLoaded()
      }

      // ////////////////////////////////////////////
      // // EXTRA CHANGES HERE:
      // ////////////////////////////////////////////
      // // NEW_CALLBACKS_AFTER_LOADING_A_MODEL
      // ////////////////////////////////////////////
      // if(true){

      //   // alert( '' + this._modelHomeDir + '\n' + 'EXTRA CHANGES HERE:' )
      ...
      //   for (let i = 0; i < this._expressions.getSize(); i++) {
      //     let aButton = window.document.createElement('button')
      //     const name: string = this._expressions._keyValues[i].first;
      //     aButton.onclick = ()=>{
      //       this.setExpression(name);
      //     }
      //     aButton.innerText = name
      ...
      //   let renderMotionGroupButtons = ( group )=>{

      //     const motionButtonsID = `motionButtons:${group}`

      //     let motionButtons = window.document.getElementById( motionButtonsID )
      //     if(motionButtons){

      //     }else{
      //       motionButtons = window.document.createElement('div')
      //       motionButtons.id = motionButtonsID
      //       window.document.body.appendChild( motionButtons )
      //     }
      //     motionButtons.innerHTML=''
      //     motionButtons.innerText=`Motions:${group}:\n`
      ...
      //         this.startMotion(group, no, LAppDefine.PriorityForce, null);
      //       }
      //       aButton.innerText = `${group}:${no}`
      //       motionButtons.appendChild(aButton)
      //     }

      //   }

      //   renderMotionGroupButtons( LAppDefine.MotionGroupIdle )
      //   renderMotionGroupButtons( LAppDefine.MotionGroupTapBody )
